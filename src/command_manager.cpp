...
constexpr std::size_t NUM_BUILTINS = sizeof(BUILTIN_NAMES) / sizeof(BUILTIN_NAMES[0]);
...
CommandID CommandManager::get_command_id(const std::string& name,
    ...
    if (name.empty()) return CommandID::EXTERNAL;
    if (string_equals_ignore_case(name, "exit")) return CommandID::EXIT;
    for (std::size_t i = 0; i < NUM_BUILTINS; ++i) {
        if (string_equals_ignore_case(name, BUILTIN_NAMES[i])) return CommandID::BUILTIN;
    }
...
bool CommandManager::is_builtin(const std::string& name) {
    for (std::size_t i = 0; i < NUM_BUILTINS; ++i) {
        if (string_equals_ignore_case(name, BUILTIN_NAMES[i])) return true;
    }
    return false;
}
